...
const LiveGpsDataFetchPage = () => {
  ...
  const [liveGpsDataFetchDetails, setliveGpsDataFetchDetails] =
  ...
  const handleNameChange = (e, s_entity_id1) => {
    ...
    if (name === 's_entity_id_and_name') {
      setliveGpsDataFetchDetails(prevData => ({
        ...prevData,
  ...
  const resetForm = () => {
    setliveGpsDataFetchDetails(initialState)
  }
  ...
  const handleSubmit = e => {
    ...
    const queryString = `?s_entity_id=${liveGpsDataFetchDetails.s_entity_id}`
    fetch(`${ubuntuIP}/api/v0/getLiveVehicleData${queryString}`, {
      method: 'GET',
      headers: {
    ...
    <div>
      <div className='wrapper'>
        <div className='container'>
          <h2>Live GPS Data</h2>
          <div className='LiveGpsDataFetchPage'>
            <form onSubmit={handleSubmit}>
              <div className='form-group'>
                <label
                  htmlFor='s_entity_id_and_name'
                  className={`required-label ${
                    liveGpsDataFetchDetails.s_entity_id_and_name
                      ? 'required'
                      : ''
                  }`}
                >
                ...
                <select
                  ...
                  required
                  value={liveGpsDataFetchDetails.s_entity_id_and_name}
                  onChange={e => {
...
export default LiveGpsDataFetchPage
